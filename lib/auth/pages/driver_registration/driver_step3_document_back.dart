...
class DriverStep3DocumentBack extends StatefulWidget {
  final Map<String, dynamic> initialData;
  final VoidCallback onNext;
  final VoidCallback onBack;

  const DriverStep3DocumentBack({
    super.key,
  ...
  @override
  State<DriverStep3DocumentBack> createState() =>
      _DriverStep3DocumentBackState();
}

class _DriverStep3DocumentBackState extends State<DriverStep3DocumentBack>
    with TickerProviderStateMixin {
  // Image file for back of ID
  File? _backImage;
  String? _backImageUrl; // URL from server after upload
  ...
  void _loadInitialData() {
    ...
    final String? backImagePath = widget.initialData['backImagePath'];
    final String? backImageUrl = widget.initialData['backImageUrl'];
  ...
  void _continuousIdScan() {
    ...
    // Check for ID back in frame every 2 seconds (Gemini API call)
    Future.delayed(const Duration(seconds: 2), () async {
      if (mounted && _isCameraInitialized && !_isCountingDown && _cameraController != null) {
        print('🔍 Gemini: Scanning for ID back in frame...');
        ...
        if (idInFrame && !_isIdDetected) {
          print('✅ Gemini: ID back detected in frame - starting 5s countdown!');
          _startCountdown();
        } else if (!idInFrame && _isIdDetected && !_isCountingDown) {
          print('❌ Gemini: ID back moved out of frame - stopping detection');
          _resetDetection();
          _continuousIdScan();
    ...
    Future.delayed(const Duration(seconds: 1), () async {
      if (mounted && _isCountingDown) {
        ...
        if (_countdown % 2 == 0 && _cameraController != null && _isCameraInitialized) {
          ...
          if (!stillInFrame) {
            print('❌ Gemini: ID back moved during countdown - canceling!');
            _resetDetection();
            _continuousIdScan();
            return;
          }
  ...
  Future<void> _autoCapture() async {
    try {
      if (_cameraController != null && _isCameraInitialized) {
        print('AI: Taking full back side camera photo...');
        ...
        print('AI: Analyzing and cropping to ID back area only...');
        ...
        if (croppedFile != null) {
          ...
          if (uploadedUrl != null) {
            ...
            widget.initialData.addAll({
              ...
              'backImageUrl': uploadedUrl, // Server URL for verification
            });

            print('✅ Step 3: Back image uploaded and saved');
            print('🌐 Image URL: $uploadedUrl');
            print('🔄 Step 3: User can now click Continue to go to Step 4');
          } else {
            // Upload failed
            TopNotification.show(
              context,
  ...
  Future<void> _pickImage(String type) async {
    print('Starting camera selection for back side');
          ...
          return SizedBox(
            ...
            child: Column(
              children: [
                ...
                // Header
                Text(
                  AppLocalizations.of(context)?.positionYourIdCardBackSide ?? 'ID Card — Back Side',
                  style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.w700,
                    color: isLight ? Colors.black : Colors.white,
  ...
  Future<File?> _cropImageToIDArea(File imageFile) async {
    try {
      print('AI: Starting to crop back image to ID rectangle area only...');
      ...
      if (image != null) {
        print('AI: Loaded back image ${image.width}x${image.height}');
        ...
        print('AI: Failed to decode back image');
      }
    } catch (e) {
      print('AI: Error cropping to back rectangle area: $e');
    }
                    ...
                    child: SizedBox(
                      ...
                      child: TradeRepublicButton(
                        label: _backImage != null
                            ? AppLocalizations.of(context)?.continueToLicense ?? 'Continue to License'
                            : AppLocalizations.of(context)?.uploadRequired ?? 'Upload Required',
                        icon: _backImage != null
                            ? Icon(CupertinoIcons.arrow_right, size: 18)
                            : null,
                        onPressed: _backImage != null
                            ? () {
                                widget.initialData.addAll({
                                  'backImagePath': _backImage?.path,
  ...
  Widget _buildIDUploadSection(bool isLight) {
    return Column(
      ...
      children: [
        ...
        // Back of ID
        _buildPhotoCapture(
          title: AppLocalizations.of(context)?.backOfId ?? 'Back of ID',
