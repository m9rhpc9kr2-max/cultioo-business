...
class DriverStep2DocumentUpload extends StatefulWidget {
  final VoidCallback onNext;
  final VoidCallback onBack;
  final Map<String, dynamic> initialData;

  const DriverStep2DocumentUpload({
    super.key,
  ...
  @override
  State<DriverStep2DocumentUpload> createState() =>
      _DriverStep2DocumentUploadState();
}

class _DriverStep2DocumentUploadState extends State<DriverStep2DocumentUpload>
    with TickerProviderStateMixin {
  // Image files
  File? _frontImage;
  String? _frontImageUrl; // URL from server after upload
  ...
  // Personal Information Form Fields - REMOVED
  // Gender, Nationality, Place of Birth are no longer collected in Step 2
  ...
  void _loadInitialData() {
    // Load any existing document data
    String? existingImagePath = widget.initialData['frontImagePath'];
    String? existingImageUrl = widget.initialData['frontImageUrl'];

    if (existingImagePath != null && existingImagePath.isNotEmpty) {
      _frontImage = File(existingImagePath);
    }

    if (existingImageUrl != null && existingImageUrl.isNotEmpty) {
      _frontImageUrl = existingImageUrl;
    }
  }
  ...
  Future<void> _pickImage(String type) async {
    print('Starting camera selection for $type');
  ...
  Future<void> _initializeCamera() async {
    ...
    try {
      ...
      // IMPORTANT: Disable flash to avoid white photo due to flash
      try {
  ...
  void _continuousIdScan() {
    ...
    Future.delayed(const Duration(seconds: 2), () async {
      if (mounted && _isCameraInitialized && !_isCountingDown && _cameraController != null) {
        ...
        if (idInFrame && !_isIdDetected) {
          print('✅ Gemini: ID card detected in frame - starting 5s countdown!');
          _startCountdown();
        } else if (!idInFrame && _isIdDetected && !_isCountingDown) {
          print('❌ Gemini: ID card moved out of frame - stopping detection');
          _resetDetection();
          _continuousIdScan();
  ...
  void _runCountdownTimer() {
    ...
    if (_countdown <= 0) {
      print('📸 Gemini: Countdown finished - taking photo!');
      _autoCapture();
      return;
    }
    ...
    Future.delayed(const Duration(seconds: 1), () async {
      if (mounted && _isCountingDown) {
        ...
        if (_countdown % 2 == 0 && _cameraController != null && _isCameraInitialized) {
          ...
          if (!stillInFrame) {
            print('❌ Gemini: ID moved during countdown - canceling!');
            _resetDetection();
            _continuousIdScan();
            return;
          }
  ...
  Future<void> _autoCapture() async {
    try {
      if (_cameraController != null && _isCameraInitialized) {
        print('AI: Taking full camera photo...');
        ...
        print('AI: Analyzing and cropping to ID frame area only...');
        // Crop the photo to ONLY the ID card area (the rectangle)
        final File? croppedFile = await _cropImageToIDArea(File(photo.path));
        ...
        if (croppedFile != null) {
          ...
          print('📤 Uploading front ID image to server...');
          ...
          if (uploadedUrl != null) {
            setState(() {
              _frontImageUrl = uploadedUrl;
            });

            // Show success notification
            TopNotification.show(
              context,
              message: AppLocalizations.of(context)?.idDocumentUploadedSuccess ?? 'ID document uploaded successfully!',
            ...
            widget.initialData.addAll({
              ...
              'frontImageUrl': uploadedUrl, // Server URL for verification
            });

            print('✅ Step 2: Front image uploaded and saved');
            print('🌐 Image URL: $uploadedUrl');
            print('🔄 Step 2: User can now click Continue to go to Step 3');
          } else {
            // Upload failed
            TopNotification.show(
              context,
              message: AppLocalizations.of(context)?.failedToUploadId ?? 'Failed to upload ID. Please try again.',
      ...
      TopNotification.show(
        context,
        message: AppLocalizations.of(context)?.errorCapturingId ?? 'Error capturing ID. Please try again.',
        type: NotificationType.error,
      );
          ...
          return SizedBox(
            ...
            child: Column(
              children: [
                ...
                // Header
                Text(
                  AppLocalizations.of(context)?.positionYourIdCard ?? 'Position Your ID Card',
                  style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.w700,
                    color: isLight ? Colors.black : Colors.white,
  ...
  Future<File?> _cropImageToIDArea(File imageFile) async {
    try {
      print('AI: Starting to crop image to ID rectangle area only...');
      ...
      if (image != null) {
        print('AI: Loaded image ${image.width}x${image.height}');

        // Calculate EXACT ID frame dimensions matching our overlay painter
        ...
        final frameHeight = (frameWidth * 0.63)
            .round(); // ID card aspect ratio 1.6:1
        ...
        // Save the cropped rectangle as the final ID image
        final tempDir = await getTemporaryDirectory();
        final croppedPath = path.join(
          tempDir.path,
          'id_rectangle_${DateTime.now().millisecondsSinceEpoch}.jpg',
        );
        ...
        print('AI: ID rectangle saved successfully to: $croppedPath');
        return croppedFile;
      } else {
        print('AI: Failed to decode image');
      }
    } catch (e) {
      print('AI: Error cropping to rectangle area: $e');
    }
  ...
  @override
  void dispose() {
    print('🧹 Disposing Step 2 resources...');
                    ...
                    child: SizedBox(
                      ...
                      child: TradeRepublicButton(
                        label: _frontImage != null
                            ? AppLocalizations.of(context)?.continueToBackSide ?? 'Continue to Back Side'
                            : AppLocalizations.of(context)?.uploadRequired ?? 'Upload Required',
                        icon: _frontImage != null
    ...
    return Center(
      child: Column(
        children: [
          ...
          Text(
            AppLocalizations.of(context)?.documentUpload ?? 'Document Upload',
            style: TextStyle(
              color: isLight ? Colors.black : Colors.white,
              fontSize: 32,
              fontWeight: FontWeight.w700,
          ...
          Text(
            '${AppLocalizations.of(context)?.stepXofY ?? 'Step'} 2 ${AppLocalizations.of(context)?.ofLabel ?? 'of'} 10 - ${AppLocalizations.of(context)?.idDocumentLabel ?? 'ID Document'}',
            style: TextStyle(
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Text(
            AppLocalizations.of(context)?.captureYourIdDocument ?? 'Capture Your ID Document',
            style: TextStyle(
              color: isLight ? Colors.black : Colors.white,
              fontSize: 18,
              fontWeight: FontWeight.w700,
        ...
        // Front of ID
        _buildPhotoCapture(
          title: AppLocalizations.of(context)?.frontOfId ?? 'Front of ID',
          subtitle: AppLocalizations.of(context)?.makeSureTextVisible ?? 'Make sure all text is clearly visible',
          image: _frontImage,
