...
class DriverStep4LicenseFront extends StatefulWidget {
  final Map<String, dynamic> initialData;
  final VoidCallback onNext;
  final VoidCallback onBack;

  const DriverStep4LicenseFront({
    super.key,
  ...
  @override
  State<DriverStep4LicenseFront> createState() =>
      _DriverStep4LicenseFrontState();
}

class _DriverStep4LicenseFrontState extends State<DriverStep4LicenseFront>
    with TickerProviderStateMixin {
  // Image files
  File? _licenseFrontImage;
  String? _licenseFrontImageUrl; // URL from server after upload
  ...
  void _loadInitialData() {
    ...
    String? existingImagePath = widget.initialData['licenseFrontImagePath'];
    String? existingImageUrl = widget.initialData['licenseFrontImageUrl'];

    if (existingImagePath != null && existingImagePath.isNotEmpty) {
      _licenseFrontImage = File(existingImagePath);
    }

    if (existingImageUrl != null && existingImageUrl.isNotEmpty) {
      _licenseFrontImageUrl = existingImageUrl;
    }
  }
  ...
  void _continuousLicenseScan() {
    ...
    // Check for license in frame every 2 seconds (Gemini API call)
    Future.delayed(const Duration(seconds: 2), () async {
      if (mounted && _isCameraInitialized && !_isCountingDown && _cameraController != null) {
        print('🔍 Gemini: Scanning for driver\'s license in frame...');
        ...
        if (licenseInFrame && !_isLicenseDetected) {
          print('✅ Gemini: Driver\'s license detected in frame - starting 5s countdown!');
          _startCountdown();
        } else if (!licenseInFrame && _isLicenseDetected && !_isCountingDown) {
          print('❌ Gemini: Driver\'s license moved out of frame - stopping detection');
          _resetDetection();
          _continuousLicenseScan();
    ...
    Future.delayed(const Duration(seconds: 1), () async {
      if (mounted && _isCountingDown) {
        ...
        if (_countdown % 2 == 0 && _cameraController != null && _isCameraInitialized) {
          ...
          if (!stillInFrame) {
            print('❌ Gemini: License moved during countdown - canceling!');
            _resetDetection();
            _continuousLicenseScan();
            return;
          }
    ...
    try {
      if (_cameraController != null && _isCameraInitialized) {
        ...
        if (croppedFile != null) {
          ...
          setState(() {
            _licenseFrontImage = croppedFile;
          });
          _resetDetection();
          ...
          if (uploadedUrl != null) {
            setState(() {
              _licenseFrontImageUrl = uploadedUrl;
            });
            ...
            widget.initialData.addAll({
              ...
              'licenseFrontImageUrl':
                  uploadedUrl, // Server URL for verification
            });

            print('✅ Step 5: License front uploaded and saved');
  ...
  Future<File?> _cropImageToLicenseArea(File imageFile) async {
    try {
      print('AI: Starting to crop image to license rectangle area only...');
      ...
      if (image != null) {
        ...
        // Calculate EXACT license frame dimensions matching our overlay painter
  ...
  void _handleNext() {
    if (_licenseFrontImage != null) {
      // Save license front image data
      widget.initialData.addAll({
        'licenseFrontImagePath': _licenseFrontImage?.path,
        'licenseFrontImageUrl': _licenseFrontImageUrl,
      });
      widget.onNext();
    } else {
      TopNotification.show(
          ...
          return SizedBox(
            ...
            child: Column(
              children: [
                ...
                // Header
                Text(
                  AppLocalizations.of(context)?.positionYourDriversLicense ?? 'Driver\'s License — Front',
                  style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.w700,
                    color: isLight ? Colors.black : Colors.white,
                ...
                children: [
                  ...
                  Expanded(
                    child: SizedBox(
                      height: 52,
                      child: TradeRepublicButton(
                              label: _licenseFrontImage != null
                                  ? AppLocalizations.of(context)?.continueToBack ?? 'Continue to Back'
                                  : AppLocalizations.of(context)?.uploadRequired ?? 'Upload Required',
                              icon: _licenseFrontImage != null
  ...
  Widget _buildLicenseUploadSection(bool isLight) {
    return Column(
      ...
      children: [
        ...
        // Front of License
        _buildPhotoCapture(
          title: AppLocalizations.of(context)?.frontOfDriversLicense ?? 'Front of Driver\'s License',
